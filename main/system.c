...
{
    GLOBAL_STATE->SYSTEM_MODULE.duration_start = esp_timer_get_time();
}
...
void SYSTEM_notify_found_nonce(GlobalState *GLOBAL_STATE, bool found_block, double found_diff)
{
    ...
    double duration = (double)(esp_timer_get_time() - module->duration_start) / 1000000.0;
    if (duration <= 0.0) {
        duration = 0.000001;
    }

    double rolling_rate = (sum * 4294967296.0) / (duration * 1000000000.0);
    if (module->historical_hashrate_init < HISTORY_LENGTH) {
