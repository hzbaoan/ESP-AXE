...
void queue_enqueue_with_count(work_queue *queue, void *new_work, int *count_after_enqueue)
{
    ...
    queue->count++;
    if (count_after_enqueue != NULL) {
        *count_after_enqueue = queue->count;
    }
...
{
    queue_enqueue_with_count(queue, new_work, NULL);
}
...
void *queue_dequeue_with_count(work_queue *queue, int *count_after_dequeue)
{
    ...
    queue->count--;
    if (count_after_dequeue != NULL) {
        *count_after_dequeue = queue->count;
    }
...
{
    return queue_dequeue_with_count(queue, NULL);
}
