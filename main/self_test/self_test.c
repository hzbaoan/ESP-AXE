...
esp_err_t init_voltage_regulator(GlobalState * GLOBAL_STATE) {
    ...
    ESP_RETURN_ON_ERROR(VCORE_set_voltage(nvs_config_get_u16_clamped(NVS_CONFIG_ASIC_VOLTAGE,
                                                                     CONFIG_ASIC_VOLTAGE,
                                                                     NVS_CONFIG_ASIC_VOLTAGE_MIN_MV,
                                                                     NVS_CONFIG_ASIC_VOLTAGE_MAX_MV) / 1000.0,
                                         GLOBAL_STATE),
                        TAG,
...
void self_test(void * pvParameters)
{
    ...
    if (!ASIC_init_job_resources(GLOBAL_STATE)) {
        ESP_LOGE(TAG, "ASIC job resource init failed!");
        tests_done(GLOBAL_STATE, TESTS_FAILED);
    }

    uint8_t chips_detected = ASIC_init(GLOBAL_STATE);
    uint8_t chips_expected = ASIC_get_expected_asic_count(GLOBAL_STATE);
    ...
    // 解析旧的十六进制为全新的二进制数据格式
    hex2bin("0c859545a3498373a57452fac22eb7113df2a465000543520000000000000000",
            notify_message.prev_block_hash_bin,
            32);
    flip32bytes(notify_message.prev_block_hash, notify_message.prev_block_hash_bin);
    memcpy(notify_message.prev_block_hash_be, notify_message.prev_block_hash_bin, sizeof(notify_message.prev_block_hash_be));
    reverse_bytes(notify_message.prev_block_hash_be, sizeof(notify_message.prev_block_hash_be));
    ...
    // 调用新版无拷贝的 Job 构造器
    bm_job job = construct_bm_job_bin(&notify_message,
                                      merkle_root_bin,
                                      0x1fffe000,
                                      1000000);
    ...
    if (!ASIC_set_job_difficulty_mask(GLOBAL_STATE, difficulty_mask)) {
        ESP_LOGE(TAG, "ASIC difficulty update failed!");
        tests_done(GLOBAL_STATE, TESTS_FAILED);
    }
    job.asic_report_difficulty = GLOBAL_STATE->ASIC_difficulty;
    job.asic_config_epoch = GLOBAL_STATE->asic_config_epoch;
    job.work_epoch = GLOBAL_STATE->work_epoch;
    ...
    while (duration < hashtest_timeout) {
        ...
        if (asic_result != NULL) {
            ...
            if (nonce_diff >= (double)difficulty_mask) {
                sum += difficulty_mask;
            }
    ...
    float expected_hashrate_mhs =
        (float)GLOBAL_STATE->POWER_MANAGEMENT_MODULE.frequency_value *
        (float)ASIC_get_small_core_count(GLOBAL_STATE) *
        (float)ASIC_get_asic_count(GLOBAL_STATE);
