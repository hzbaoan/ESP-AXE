...
interface PoolInformationView {
  role: 'Primary' | 'Fallback';
  protocolLabel: 'Stratum V1' | 'Stratum V2';
  host: string;
  port: number;
  user: string;
  quickLink?: string;
...
export class HomeComponent {
  ...
  public poolInformation$!: Observable<PoolInformationView>;
  ...
  private initializeChart() {
    ...
    this.poolInformation$ = this.info$.pipe(
      map(info => this.getPoolInformation(info))
    );
  ...
  private getPoolInformation(info: ISystemInfo): PoolInformationView {
    const usingFallback = !!info.isUsingFallbackStratum;
    const protocol = usingFallback ? info.fallbackStratumProtocol : info.stratumProtocol;
    const isSv2 = Number(protocol) === 2;
    const host = usingFallback
      ? (isSv2 ? info.fallbackSv2Host : info.fallbackStratumURL)
      : (isSv2 ? info.sv2Host : info.stratumURL);
    const port = usingFallback
      ? (isSv2 ? info.fallbackSv2Port : info.fallbackStratumPort)
      : (isSv2 ? info.sv2Port : info.stratumPort);
    const user = usingFallback ? info.fallbackStratumUser : info.stratumUser;

    return {
      role: usingFallback ? 'Fallback' : 'Primary',
      protocolLabel: isSv2 ? 'Stratum V2' : 'Stratum V1',
      host,
      port,
      user,
      quickLink: this.getQuickLink(host, user)
    };
  }

  private getQuickLink(poolHost: string, stratumUser: string): string | undefined {
    const address = stratumUser.split('.')[0];

    if (poolHost.includes('public-pool.io')) {
      return `https://web.public-pool.io/#/app/${address}`;
    } else if (poolHost.includes('ocean.xyz')) {
      return `https://ocean.xyz/stats/${address}`;
    } else if (poolHost.includes('solo.d-central.tech')) {
      return `https://solo.d-central.tech/#/app/${address}`;
    } else if (/^eusolo[46]?.ckpool.org/.test(poolHost)) {
      return `https://eusolostats.ckpool.org/users/${address}`;
    } else if (/^solo[46]?.ckpool.org/.test(poolHost)) {
      return `https://solostats.ckpool.org/users/${address}`;
    } else if (poolHost.includes('pool.noderunners.network')) {
      return `https://noderunners.network/en/pool/user/${address}`;
    } else if (poolHost.includes('satoshiradio.nl')) {
      return `https://pool.satoshiradio.nl/user/${address}`;
    } else if (poolHost.includes('solohash.co.uk')) {
      return `https://solohash.co.uk/user/${address}`;
    }
    return poolHost.startsWith('http') ? poolHost : `http://${poolHost}`;
  }
