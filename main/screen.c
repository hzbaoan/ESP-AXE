...
static void screen_update_cb(lv_timer_t * timer)
{
    ...
    double observed_hashrate = SYSTEM_get_current_hashrate(GLOBAL_STATE);
    ...
    if (current_hashrate != observed_hashrate) {
        lv_label_set_text_fmt(hashrate_label, "Gh/s: %.2f", observed_hashrate);
    }

    if (current_power != power_management->power || current_hashrate != observed_hashrate) {
        if (power_management->power > 0 && observed_hashrate > 0) {
            float efficiency = power_management->power / (observed_hashrate / 1000.0);
    ...
    current_hashrate = observed_hashrate;
