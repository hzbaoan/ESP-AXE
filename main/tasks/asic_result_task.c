...
void ASIC_result_task(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        if (is_share)
        {
            uint32_t version_bits = submit_version_bits(asic_result->rolled_version, active_job.submit_version_mask);
            uint32_t submit_version = GLOBAL_STATE->SYSTEM_MODULE.is_using_fallback ?
                (GLOBAL_STATE->SYSTEM_MODULE.fallback_pool_protocol == STRATUM_PROTOCOL_V2 ?
                 asic_result->rolled_version : version_bits) :
            ...
            if (!should_submit) {
                ESP_LOGW(TAG, "Dropping duplicate share: id=%s nonce=%08" PRIX32 " ver=%08" PRIX32,
                         active_job.jobid, asic_result->nonce, submit_version);
            } else {
                strncpy(share.username, user != NULL ? user : "", sizeof(share.username) - 1);
                strncpy(share.jobid, active_job.jobid, sizeof(share.jobid) - 1);
                strncpy(share.extranonce_2, active_job.extranonce2, sizeof(share.extranonce_2) - 1);
                share.ntime = active_job.ntime;
                share.nonce = asic_result->nonce;
                share.version = submit_version;

                if (!stratum_queue_share(GLOBAL_STATE, &share)) {
                    ESP_LOGW(TAG,
                             "Share queue full, dropping share: id=%s nonce=%08" PRIX32 " ver=%08" PRIX32,
                             active_job.jobid,
                             asic_result->nonce,
                             submit_version);
                }
