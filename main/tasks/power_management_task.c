...
// ======== 新增：热身参数 ========
#define WARMUP_VOLTAGE_MV 1250   // 热身强行锁定的电压值 1250mV
#define WARMUP_DURATION_MS 60000 // 热身持续时间 60秒 (60000 毫秒)
// ================================
...
void POWER_MANAGEMENT_task(void * pvParameters)
{
    ...
    TickType_t startup_time = xTaskGetTickCount();
    // ======================================
    ...
    while (1) {
        ...
        uint16_t user_core_voltage = nvs_config_get_u16(NVS_CONFIG_ASIC_VOLTAGE, CONFIG_ASIC_VOLTAGE);
        ...
        if (is_warming_up) {
            ...
            if ((current_time - startup_time) * portTICK_PERIOD_MS < WARMUP_DURATION_MS) {
                // 如果用户设置的电压低于热身电压，则在热身期内强制提升到热身电压
                if (user_core_voltage < WARMUP_VOLTAGE_MV) {
                    target_core_voltage = WARMUP_VOLTAGE_MV;
                }
            } else {
                // 热身时间结束，状态复位
                is_warming_up = false;
                ESP_LOGI(TAG, "Warm-up complete. Reverting ASIC voltage to user config: %umV", user_core_voltage);
            }
        }
        // ======================================

        if (target_core_voltage != last_core_voltage) {
            if (is_warming_up && target_core_voltage == WARMUP_VOLTAGE_MV) {
                ESP_LOGI(TAG, "WARM-UP ACTIVE: Forcing vcore voltage to %umV", target_core_voltage);
            } else {
                ESP_LOGI(TAG, "Setting new vcore voltage to %umV", target_core_voltage);
            }
            
            VCORE_set_voltage((double) target_core_voltage / 1000.0, GLOBAL_STATE);
