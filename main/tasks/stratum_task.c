...
static bool stratum_primary_probe(GlobalState * GLOBAL_STATE)
{
    ...
    if (setsockopt(sock, SOL_SOCKET, SO_SNDTIMEO, &tcp_timeout, sizeof(tcp_timeout)) != 0) {
        ESP_LOGE(TAG, "Primary probe failed to set SO_SNDTIMEO");
    }

    if (setsockopt(sock, SOL_SOCKET, SO_RCVTIMEO, &tcp_timeout, sizeof(tcp_timeout)) != 0) {
        ESP_LOGE(TAG, "Primary probe failed to set SO_RCVTIMEO");
    }
