...
void BM1397_set_version_mask(uint32_t version_mask) {
    uint32_t effective_mask = version_mask & STRATUM_DEFAULT_VERSION_MASK;
    uint8_t midstate_slots = version_rolling_mask_slots(0, effective_mask, 4);

    if (effective_mask == 0) {
        ESP_LOGI(TAG, "BM1397 version rolling disabled");
        return;
    }

    ESP_LOGI(TAG,
             "BM1397 version rolling configured for %" PRIu8 " rolled versions per job (mask %08" PRIX32 ")",
             midstate_slots,
             effective_mask);
}
...
task_result *BM1397_process_work(void *pvParameters)
{
    ...
    if (!recent_result_cache_add(&recent_results, asic_result.job_id, asic_result.nonce, 0, asic_result.midstate_num)) {
        return NULL;
    }
    ...
    GlobalState *GLOBAL_STATE = (GlobalState *)pvParameters;
    bm_job active_job;
    if (!ASIC_copy_active_job(GLOBAL_STATE, rx_job_id, &active_job)) {
        ESP_LOGW(TAG, "Invalid job nonce found, id=%d", rx_job_id);
        return NULL;
    }

    uint32_t rolled_version = active_job.version;
    for (int i = 0; i < rx_midstate_index; i++)
    {
        rolled_version = increment_bitmask(rolled_version, active_job.version_mask);
    }
