...
static bool _send_BM1368(uint8_t header, uint8_t * data, uint8_t data_len, bool debug)
{
    packet_type_t packet_type = (header & TYPE_JOB) ? JOB_PACKET : CMD_PACKET;
    uint8_t total_length = (packet_type == JOB_PACKET) ? (data_len + 6) : (data_len + 5);
    int sent;

    // [黄金优化]: 废弃 malloc，采用极速栈内存
    uint8_t buf[128];
...
static bool _send_simple(const uint8_t *data, uint8_t total_length)
{
    ...
    sent = SERIAL_send(buf, total_length, BM1368_SERIALTX_DEBUG);
    if (sent != total_length) {
...
static bool _send_chain_inactive(void)
{
    ...
    return _send_BM1368((TYPE_CMD | GROUP_ALL | CMD_INACTIVE), read_address, 2, BM1368_SERIALTX_DEBUG);
}

static bool _set_chip_address(uint8_t chipAddr)
{
    unsigned char read_address[2] = {chipAddr, 0x00};
    return _send_BM1368((TYPE_CMD | GROUP_SINGLE | CMD_SETADDRESS), read_address, 2, BM1368_SERIALTX_DEBUG);
}

bool BM1368_set_version_mask(uint32_t version_mask)
{
    ...
    return _send_BM1368(TYPE_CMD | GROUP_ALL | CMD_WRITE,
                        version_cmd, 6, BM1368_SERIALTX_DEBUG);
}

static bool BM1368_set_hash_counting_number(uint32_t hcn)
{
    uint8_t command[6] = {
        0x00,
        0x10,
...
bool BM1368_set_nonce_space(float frequency, uint16_t asic_count, double nonce_scale)
{
    uint32_t hcn;

    if (!calculate_hcn(frequency,
                       (uint16_t)BM1368_CORE_COUNT,
                       asic_count,
                       0,
                       nonce_scale,
                       &hcn)) {
...
float BM1368_send_hash_frequency(float target_freq) {
    ...
    if (!calculate_pll_parameters(target_freq, 144, 235,
    ...
    if (!_send_BM1368(TYPE_CMD | GROUP_ALL | CMD_WRITE, freqbuf, sizeof(freqbuf), BM1368_SERIALTX_DEBUG)) {
        return 0.0f;
    }
...
static bool do_frequency_ramp_up(float target_frequency, float *actual_frequency) {
    ESP_LOGI(TAG, "Ramping up frequency to %.2f MHz", target_frequency);
    return do_frequency_transition(target_frequency, BM1368_send_hash_frequency, 1368, actual_frequency);
}

uint8_t BM1368_init(float frequency, uint16_t asic_count, float *actual_frequency)
{
    ...
    *actual_frequency = applied_frequency;
    ...
    while (true) {
        if (SERIAL_rx(temp_buff, 11, 1500) <= 0) break;
    }

    for (int i = 0; i < 4; i++) {
        if (!BM1368_set_version_mask(STRATUM_DEFAULT_VERSION_MASK)) {
            ESP_LOGE(TAG, "Initialization failed while setting the version mask");
            return 0;
        }
    }

    if (!_send_BM1368(TYPE_CMD | GROUP_ALL | CMD_READ, (uint8_t[]){0x00, 0x00}, 2, false)) {
    ...
    for (int i = 0; i < sizeof(init_cmds) / sizeof(init_cmds[0]); i++) {
        if (!_send_BM1368(TYPE_CMD | GROUP_ALL | CMD_WRITE, init_cmds[i], 6, false)) {
            ESP_LOGE(TAG, "Initialization failed while sending global register command %u", (unsigned int)i);
            return 0;
        }
    }
    ...
    for (int i = 0; i < chip_counter; i++) {
        uint8_t chip_init_cmds[][6] = {
            {(uint8_t)(i * address_interval), 0xA8, 0x00, 0x07, 0x01, 0xF0},
            {(uint8_t)(i * address_interval), 0x18, 0xF0, 0x00, 0xC1, 0x00},
            {(uint8_t)(i * address_interval), 0x3C, 0x80, 0x00, 0x8b, 0x00},
            {(uint8_t)(i * address_interval), 0x3C, 0x80, 0x00, 0x80, 0x18},
            {(uint8_t)(i * address_interval), 0x3C, 0x80, 0x00, 0x82, 0xAA}
        };

        for (int j = 0; j < sizeof(chip_init_cmds) / sizeof(chip_init_cmds[0]); j++) {
            if (!_send_BM1368(TYPE_CMD | GROUP_SINGLE | CMD_WRITE, chip_init_cmds[j], 6, false)) {
                ESP_LOGE(TAG, "Initialization failed while setting chip %u register command %u",
    ...
    if (!BM1368_set_job_difficulty_mask(BM1368_ASIC_DIFFICULTY)) {
    ...
    if (!BM1368_set_nonce_space(*actual_frequency, (uint16_t)chip_counter, 1.0)) {
        return 0;
    }
    if (!BM1368_set_version_mask(STRATUM_DEFAULT_VERSION_MASK)) {
        ESP_LOGE(TAG, "Initialization failed while applying the final version mask");
        return 0;
    }
...
bool BM1368_set_job_difficulty_mask(uint32_t difficulty)
{
    ...
    ESP_LOGI(TAG, "Setting job ASIC mask to %" PRIu32, difficulty);
    return _send_BM1368((TYPE_CMD | GROUP_ALL | CMD_WRITE),
                        job_difficulty_mask, 6, BM1368_SERIALTX_DEBUG);
}
...
task_result * BM1368_process_work(void * pvParameters)
{
    ...
    ESP_LOGD(TAG, "Chip: %d, Job ID: %02X, Core: %d/%d, Ver: %08" PRIX32, asic_nr+1, job_id, core_id, small_core_id, version_bits);
    ...
    if(norceCount%20==0){
        ...
        ESP_LOGD(TAG, "Asic Submit Count: %s", (char*)(GLOBAL_STATE->chip_submit_srt));
    }
