...
#define CONFIG_ASIC_DISPATCH_SAFETY_MARGIN_PERCENT 5
#endif
                                                ...
                                                double *observed_hashrate_hps_out)
{
    ...
    double effective_hashrate_hps = nominal_hashrate_hps;
    bool use_observed = false;

    if (observed_hashrate_hps > 0.0 && nominal_hashrate_hps > 0.0) {
        double threshold_factor =
            (100.0 - (double)CONFIG_ASIC_DISPATCH_OBSERVED_HASHRATE_MIN_DELTA_PERCENT) / 100.0;

        if (observed_hashrate_hps < (nominal_hashrate_hps * threshold_factor)) {
            effective_hashrate_hps = observed_hashrate_hps;
            use_observed = true;
        }
    }

    if (using_observed_hashrate != NULL) {
        *using_observed_hashrate = use_observed;
    }
    if (observed_hashrate_hps_out != NULL) {
        *observed_hashrate_hps_out = observed_hashrate_hps;
    }

    return effective_hashrate_hps;
}
