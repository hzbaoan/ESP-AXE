...
static bool _send_BM1366(uint8_t header, uint8_t * data, uint8_t data_len, bool debug)
{
    packet_type_t packet_type = (header & TYPE_JOB) ? JOB_PACKET : CMD_PACKET;
    uint8_t total_length = (packet_type == JOB_PACKET) ? (data_len + 6) : (data_len + 5);
    int sent;

    // [黄金优化]: 废弃耗时的 malloc，直接使用 128 字节静态栈内存
    uint8_t buf[128];
...
static bool _send_simple(const uint8_t *data, uint8_t total_length)
{
    ...
    sent = SERIAL_send(buf, total_length, BM1366_SERIALTX_DEBUG);
    if (sent != total_length) {
...
static bool _send_chain_inactive(void)
{
    ...
    return _send_BM1366((TYPE_CMD | GROUP_ALL | CMD_INACTIVE), read_address, 2, BM1366_SERIALTX_DEBUG);
}

static bool _set_chip_address(uint8_t chipAddr)
{
    unsigned char read_address[2] = {chipAddr, 0x00};
    return _send_BM1366((TYPE_CMD | GROUP_SINGLE | CMD_SETADDRESS), read_address, 2, BM1366_SERIALTX_DEBUG);
}
...
bool BM1366_set_version_mask(uint32_t version_mask)
{
    ...
    return _send_BM1366(TYPE_CMD | GROUP_ALL | CMD_WRITE,
                        version_cmd, 6, BM1366_SERIALTX_DEBUG);
}

static bool BM1366_set_hash_counting_number(uint32_t hcn)
{
    uint8_t command[6] = {
        0x00,
        0x10,
...
float BM1366_send_hash_frequency(float target_freq)
{
    ...
    if (!_send_BM1366((TYPE_CMD | GROUP_ALL | CMD_WRITE), freqbuf, 6, BM1366_SERIALTX_DEBUG)) {
        return 0.0f;
    }
...
static bool do_frequency_ramp_up(float target_frequency, float *actual_frequency) {
    ...
    return do_frequency_transition(target_frequency, BM1366_send_hash_frequency, 1366, actual_frequency);
}

static uint8_t _send_init(float frequency, uint16_t asic_count, float *actual_frequency)
{
    for (int i = 0; i < 3; i++) {
        if (!BM1366_set_version_mask(STRATUM_DEFAULT_VERSION_MASK)) {
    ...
    if (!_send_simple(init136, 11)) {
        ESP_LOGE(TAG, "Initialization failed while setting register 0x3C (2)");
        return 0;
    }

    if (!BM1366_set_job_difficulty_mask(BM1366_ASIC_DIFFICULTY)) {
    ...
    if (!_send_simple(init171, 11)) {
        ESP_LOGE(TAG, "Initialization failed while setting register 0x2C");
        return 0;
    }

    for (uint8_t i = 0; i < chip_counter; i++) {
        unsigned char set_a8_register[6] = {(uint8_t)(i * address_interval), 0xA8, 0x00, 0x07, 0x01, 0xF0};
        if (!_send_BM1366((TYPE_CMD | GROUP_SINGLE | CMD_WRITE), set_a8_register, 6, BM1366_SERIALTX_DEBUG)) {
        ...
        if (!_send_BM1366((TYPE_CMD | GROUP_SINGLE | CMD_WRITE), set_18_register, 6, BM1366_SERIALTX_DEBUG)) {
            ESP_LOGE(TAG, "Initialization failed while setting chip %u register 0x18", (unsigned int)i);
            return 0;
        }
        unsigned char set_3c_register_first[6] = {(uint8_t)(i * address_interval), 0x3C, 0x80, 0x00, 0x85, 0x40};
        if (!_send_BM1366((TYPE_CMD | GROUP_SINGLE | CMD_WRITE), set_3c_register_first, 6, BM1366_SERIALTX_DEBUG)) {
            ESP_LOGE(TAG, "Initialization failed while setting chip %u register 0x3C (1)", (unsigned int)i);
            return 0;
        }
        unsigned char set_3c_register_second[6] = {(uint8_t)(i * address_interval), 0x3C, 0x80, 0x00, 0x80, 0x20};
        if (!_send_BM1366((TYPE_CMD | GROUP_SINGLE | CMD_WRITE), set_3c_register_second, 6, BM1366_SERIALTX_DEBUG)) {
        ...
        if (!_send_BM1366((TYPE_CMD | GROUP_SINGLE | CMD_WRITE), set_3c_register_third, 6, BM1366_SERIALTX_DEBUG)) {
    ...
    if (!BM1366_set_nonce_space(*actual_frequency, (uint16_t)chip_counter, 1.0)) {
        return 0;
    }

    unsigned char init795[11] = {0x55, 0xAA, 0x51, 0x09, 0x00, 0xA4, 0x90, 0x00, 0xFF, 0xFF, 0x1C};
    if (!_send_simple(init795, 11)) {
...
int BM1366_set_max_baud(void)
{
    ...
    return _send_simple(reg28, 11) ? 1000000 : 0;
}
...
bool BM1366_set_job_difficulty_mask(uint32_t difficulty)
{
    ...
    return _send_BM1366((TYPE_CMD | GROUP_ALL | CMD_WRITE),
                        job_difficulty_mask, 6, BM1366_SERIALTX_DEBUG);
}
