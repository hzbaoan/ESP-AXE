...
TEST_CASE("SV2 extended channel enforces extranonce2 range", "[stratum_v2]")
{
    const uint16_t extranonce_sizes[] = {
        EXTRANONCE2_MIN_BYTES - 1U,
        EXTRANONCE2_MIN_BYTES,
        EXTRANONCE2_MAX_BYTES,
        EXTRANONCE2_MAX_BYTES + 1U,
    };

    for (size_t i = 0; i < sizeof(extranonce_sizes) / sizeof(extranonce_sizes[0]); i++) {
        ...
        put_u32_le(payload, &pos, 42);
        put_bytes(payload, &pos, target, sizeof(target));
        put_u16_le(payload, &pos, extranonce_sizes[i]);
        put_u8(payload, &pos, 0);
        put_u32_le(payload, &pos, 99);

        bool parsed = STRATUM_V2_parse_open_extended_success(&channel, payload, pos, 2);
        bool supported = extranonce_sizes[i] >= EXTRANONCE2_MIN_BYTES &&
                         extranonce_sizes[i] <= EXTRANONCE2_MAX_BYTES;

        TEST_ASSERT_EQUAL(supported, parsed);
        TEST_ASSERT_EQUAL(supported, channel.valid);
    }
}
