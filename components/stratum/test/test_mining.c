...
TEST_CASE("Fractional pool target rejects the reported low difficulty share", "[mining]")
{
    const uint8_t expected_target[32] = {
        0xd6, 0x63, 0x14, 0x6a, 0x07, 0xfd, 0x01, 0x4d,
        0xcd, 0x61, 0x10, 0xaf, 0x89, 0x85, 0xd9, 0x36,
        0xfe, 0x0f, 0x66, 0x5d, 0x37, 0xb3, 0x67, 0x40,
        0xc6, 0xc7, 0x27, 0x00, 0x00, 0x00, 0x00, 0x00,
    };
    uint8_t assigned_target[32];
    uint8_t rejected_share_target[32];

    difficulty_double_to_target_le(1647.4206230108975, assigned_target);
    difficulty_double_to_target_le(1647.3069537247213, rejected_share_target);

    TEST_ASSERT_EQUAL_UINT8_ARRAY(expected_target,
                                  assigned_target,
                                  sizeof(assigned_target));
    TEST_ASSERT_FALSE(hash_meets_target(rejected_share_target, assigned_target));
}

TEST_CASE("Exact target conversion preserves integer difficulties", "[mining]")
{
    uint8_t integer_target[32];
    uint8_t exact_target[32];

    difficulty_to_target_le(1647, integer_target);
    difficulty_double_to_target_le(1647.0, exact_target);

    TEST_ASSERT_EQUAL_UINT8_ARRAY(integer_target, exact_target, sizeof(exact_target));
}

TEST_CASE("Exact target conversion remains strict above the ASIC integer limit", "[mining]")
{
    uint8_t limited_target[32];
    uint8_t exact_target[32];

    difficulty_to_target_le(UINT32_MAX, limited_target);
    difficulty_double_to_target_le((double)UINT32_MAX + 0.5, exact_target);

    TEST_ASSERT_TRUE(hash_meets_target(exact_target, limited_target));
    TEST_ASSERT_FALSE(hash_meets_target(limited_target, exact_target));
}
