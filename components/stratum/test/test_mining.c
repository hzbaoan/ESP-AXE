...
static bm_job construct_bm_job(mining_notify *params, const char *merkle_root_hex, uint32_t version_mask, uint32_t difficulty)
{
    ...
    return construct_bm_job_bin(params, merkle_root_bin, version_mask, 4, difficulty);
}
...
TEST_CASE("Version rolling window helpers enumerate masked slots", "[mining]")
{
    ...
    TEST_ASSERT_EQUAL_UINT32(4, version_rolling_total_slots(version_mask));
    TEST_ASSERT_EQUAL_UINT32(2, version_rolling_window_count(version_mask, 2));
...
TEST_CASE("Windowed bm job construction normalizes version and tracks window base", "[mining]")
{
    mining_notify notify_message = {0};
    uint8_t merkle_root_bin[32] = {0};
    bm_job job = {0};

    notify_message.version = 0x2000A004;
    notify_message.target = 0x1705dd01;
    notify_message.ntime = 0x64658bd8;
    ...
    TEST_ASSERT_EQUAL_HEX32(0x20008004, job.version);
    TEST_ASSERT_EQUAL_HEX32(2, job.version_window_base_index);
    TEST_ASSERT_EQUAL_UINT8(2, job.version_window_slots);
    TEST_ASSERT_EQUAL_UINT8(2, job.num_midstates);
}
