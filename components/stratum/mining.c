...
static uint64_t divide_u256_be_by_u64(const uint8_t numerator[32], uint64_t divisor,
                                      uint8_t quotient[32])
{
    uint8_t remainder[9] = {0};
    uint8_t divisor_be[9] = {0};
    uint64_t remainder_u64 = 0;
    ...
    for (int i = 1; i < 9; i++) {
        remainder_u64 = (remainder_u64 << 8) | remainder[i];
    }
    return remainder_u64;
}

static void shift_u256_be_left(uint8_t value[32], unsigned int shift)
{
    while (shift-- > 0) {
        uint8_t carry = 0;

        for (int i = 31; i >= 0; i--) {
            uint8_t next_carry = value[i] >> 7;
...
static void shift_u256_be_right(uint8_t value[32], unsigned int shift)
{
    if (shift >= 256) {
        memset(value, 0, 32);
        return;
...
static void add_u64_to_u256_be(uint8_t value[32], uint64_t addend)
{
    uint16_t carry = 0;

    for (int i = 31; i >= 0 && (addend != 0 || carry != 0); i--) {
        uint16_t sum = (uint16_t)value[i] + (uint8_t)addend + carry;
        value[i] = (uint8_t)sum;
        carry = sum >> 8;
        addend >>= 8;
    }
...
void difficulty_double_to_target_le(double difficulty, uint8_t target[32])
{
    ...
    memcpy(&representation, &difficulty, sizeof(representation));
    if (difficulty > (double)UINT32_MAX ||
            difficulty != (double)(uint32_t)difficulty) {
        representation++;
    }
    exponent = (int)((representation >> 52) & 0x7ffU) - 1023;
    significand = (representation & UINT64_C(0x000fffffffffffff)) |
                  UINT64_C(0x0010000000000000);

    // difficulty = significand * 2^(exponent - 52). Keep the division in
    // integers so the resulting 256-bit target can be compared without loss.
    remainder = divide_u256_be_by_u64(diff1_target_be, significand, quotient_be);
    if (exponent > 52) {
        shift_u256_be_right(quotient_be, (unsigned int)(exponent - 52));
        reverse_copy_32(quotient_be, target);
        return;
    }

    fractional_bits = (unsigned int)(52 - exponent);
    shift_u256_be_left(quotient_be, fractional_bits);
    for (unsigned int i = 0; i < fractional_bits; i++) {
        remainder <<= 1;
        fractional_quotient <<= 1;
    ...
    add_u64_to_u256_be(quotient_be, fractional_quotient);
    reverse_copy_32(quotient_be, target);
}
