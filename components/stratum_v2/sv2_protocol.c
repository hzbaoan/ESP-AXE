...
static double s_le256todouble(const void *target)
{
    ...
    dcut64 = (double)read_u64_le((const uint8_t *)target + 24) * s_bits192;
    dcut64 += (double)read_u64_le((const uint8_t *)target + 16) * s_bits128;
    dcut64 += (double)read_u64_le((const uint8_t *)target + 8) * s_bits64;
    dcut64 += (double)read_u64_le((const uint8_t *)target);
...
int sv2_parse_reconnect(const uint8_t *payload, uint32_t len,
                        char *host, size_t host_size,
                        uint16_t *port)
{
    if (host == NULL || host_size == 0 || port == NULL) return -1;

    int consumed = read_str0255(payload, len, host, host_size);
    if (consumed < 0) return -1;
    if ((uint32_t)consumed + 2 > len) return -1;

    *port = read_u16_le(payload + consumed);
    return 0;
}
                                  ...
                                  char *error_code, size_t error_code_size)
{
    ...
    if (len < 9) return -1;
    if (error_code == NULL || error_code_size == 0) return -1;
                                                  ...
                                                  uint32_t *channel_id_out)
{
    ...
    job->min_ntime_set = has_min_ntime;
