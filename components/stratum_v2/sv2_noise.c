...
static int sv2_noise_recv_internal(sv2_noise_ctx_t *ctx,
                                   ...
                                   int timeout_ms)
{
    if (!ctx || !ctx->handshake_complete || transport == NULL || timeout_ms <= 0) {
        return -1;
    }
    ...
    *payload_len_out = 0;
    ...
    uint8_t enc_hdr[22];
    if (noise_recv_exact(transport, enc_hdr, 22, timeout_ms) != 0) {
        return -1;
    }
    ...
    if (noise_recv_exact(transport, ctx->recv_buf, enc_len, timeout_ms) != 0) {
        return -1;
    }
...
{
    return sv2_noise_recv_internal(ctx,
                                   transport,
                                   hdr_out,
                                   payload_out,
                                   max_payload_len,
