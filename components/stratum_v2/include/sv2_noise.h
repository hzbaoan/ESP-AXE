...
typedef enum {
    // Verify the authority signature, but do not enforce certificate time bounds.
    SV2_CERT_TIME_CHECK_SKIP = 0,
    // Enforce certificate time bounds against time(NULL), with protocol leeway.
    // Use only when the system clock comes from a trusted source such as SNTP/RTC.
    SV2_CERT_TIME_CHECK_SYSTEM_CLOCK = 1,
} sv2_cert_time_check_t;
...
// Perform Noise_NX handshake as initiator over the given connected transport.
...
// cert_time_check controls whether certificate valid_from/not_valid_after are
// enforced against the local system clock.
...
// Receive with a caller-provided timeout. Intended for bounded health probes.
int sv2_noise_recv_timeout(sv2_noise_ctx_t *ctx, esp_transport_handle_t transport,
                           uint8_t hdr_out[6], uint8_t *payload_out,
                           int max_payload_len, int *payload_len_out,
                           int timeout_ms);
