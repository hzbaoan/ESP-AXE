...
static bool scan_reconnect_required = false;
static uint16_t ap_number = 0;
static wifi_ap_record_t ap_info[MAX_AP_COUNT];
static bool disable_startup_ap_on_connect = false;
...
esp_err_t wifi_scan(wifi_ap_record_simple_t *ap_records, uint16_t *ap_count)
{
    ...
    if (err != ESP_OK) {
        ...
        is_scanning = false;
        if (scan_reconnect_required) {
            scan_reconnect_required = false;
            esp_wifi_connect();
        }
        return err;
    }
...
static void event_handler(void * arg, esp_event_base_t event_base, int32_t event_id, void * event_data)
{
        ...
        if (event_id == WIFI_EVENT_SCAN_DONE) {
            ...
            if (scan_reconnect_required) {
                ...
                if (connect_err != ESP_OK) {
                    ESP_LOGW(TAG, "Post-scan Wi-Fi reconnect failed: %s",
    ...
    if (event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP) {
        ...
        if (disable_startup_ap_on_connect) {
            disable_startup_ap_on_connect = false;
            wifi_softap_off();
        }
    }
}
...
void toggle_wifi_softap(void)
{
    ...
    // A manual button action owns the AP state from now on. A later STA
    // reconnect must not undo the user's explicit choice.
    disable_startup_ap_on_connect = false;
...
void wifi_init(const char * wifi_ssid, const char * wifi_pass, const char * hostname, char * ip_addr_str)
{
    ...
    if (strlen(wifi_ssid) == 0) {
        ...
        disable_startup_ap_on_connect = true;
        /* Initialize STA */
...
EventBits_t wifi_connect(TickType_t timeout_ticks)
{
    ...
    if ((bits & (WIFI_CONNECTED_BIT | WIFI_FAIL_BIT)) == 0) {
        ESP_LOGW(TAG, "Initial Wi-Fi wait timed out");
    }
